  ...
  return (
    <LocaleLink
      ...
      className={cn(
        ...
        'font-sans text-micro11 font-bold uppercase tracking-wide transition-colors',
    ...
    >
      ...
      {typeName(type, lang)}
      {mult && <span className="font-display text-micro10 tabular-nums">{mult}</span>}
    </LocaleLink>
  );
}
...
export function TypeChipRow({
  ...
  if (types.length === 0) return <span className="text-micro12 font-medium text-tx-muted">{empty}</span>;
  return (
