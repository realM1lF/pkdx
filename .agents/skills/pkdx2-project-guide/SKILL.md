---
name: pkdx2-project-guide
description: >
  Technical onboarding and working agreement for the Pokédex 2.0 webapp
  (React 19 + Vite + TypeScript + Tailwind v3). Use whenever an AI coding
  agent works in this repository: implementing features, fixing bugs,
  extending the Pokédex/Maps/Nuzlocke/Team Builder/Versus pages, touching
  the EN/DE i18n system, the URL language routing (/de, /en), the design
  system, or any data pipeline (PokéAPI, @smogon/calc, @pkmn/data,
  Supabase, pret/pokefirered artifacts). Contains the project's binding
  invariants, i18n rules, design tokens, and verification workflow.
  Do NOT use for: unrelated projects, content writing, or design mockups
  outside this repository.
---

# Pokédex 2.0 — Project Guide for AI Agents

**Project-local only** — lives in `.agents/skills/pkdx2-project-guide/`. Loaded via repo-root `AGENTS.md` / `CLAUDE.md` when this repository is the workspace (not installed globally).
...
- `references/architecture.md` — data layer, caching, calc/legality libs,
...
- `references/i18n.md` — the full translation system: artifacts, lookup
  layer, bilingual search, redirect matrix, hreflang/SEO, how to add a
  string or a whole page, known English-only remainders.
- `references/design-system.md` — Holo-Dex tokens, typography, density
